...
use ethers::types::{H256, Bytes, Address, TransactionRequest};
...
pub trait EthApi {
    ...
    async fn get_balance(&self, address: Address, block_number: Option<String>) -> Result<String, ErrorObjectOwned>;
...
pub async fn run_server(_ctx: Arc<RpcContext>, _addr: SocketAddr) -> Result<(ServerHandle, SocketAddr), Box<dyn std::error::Error + Send + Sync>> {
    let server = Server::builder().build(_addr).await?;
